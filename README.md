...
Para garantir resiliência nível produção e eliminar interrupções abruptas, vazamentos de recursos ou perda de dados, o sistema implementa os seguintes mecanismos avançados:

*   **IDs Unificados Frontend/Backend (Fim das Duplicações):** O frontend gera um ID único e definitivo para a mensagem (`msg_...`) no momento do clique. Esse ID é enviado no payload do `/chat` e usado pelo banco de dados SQLite simulado do backend. Isso elimina conflitos de chaves temporárias (`temp_...`) e impede duplicidade de balões de mensagens no React e no histórico persistido.
*   **Abatimento Ativo de Streams Órfãos (Gargalo de Concorrência):** Ao enviar uma nova mensagem ou trocar de sessão no meio de uma geração, o frontend aciona o método `.abort()` do `AbortController` ativo. O backend escuta essa interrupção e limpa imediatamente o `streamState` correspondente no mapa de controle (`activeStreams`). Adicionalmente, se houver um stream ativo pendente para aquela mesma conversa, o backend aborta a requisição de origem para o llama.cpp local usando sinais nativos de abortagem, economizando CPU preciosa no servidor.
*   **Keep-Alive Heartbeat (Prevenção de Timeout em Cloud Run/Nginx):** Para conexões longas rodando sob CPU lenta, proxies como o Nginx ou o Cloud Run podem cortar canais inativos. O backend inicia um timer que envia uma linha SSE padrão `:` (linha de comentário silenciosa SSE) a cada 15 segundos. Isso mantém o canal de rede ativamente aberto sem interferir na saída visual de dados do usuário.
*   **Tratamento Seguro de Desconexão e Rehidratação:**
    *   No Backend: Se o cliente interromper a requisição, o evento `req.on("close")` é capturado. O servidor para a transmissão, finaliza o timer de keep-alive e persiste o progresso parcial acumulado do texto de resposta no banco em vez de descarta-lo.
    *   No Frontend: O estado de streaming é consumido de maneira tolerante a falhas. Caso o cliente aborte a requisição ou ocorra queda de energia, a resposta textual já renderizada é preservada de forma visual na UI sob uma anotação clara de interrupção (ex: `*🛑 Stream interrompido pelo usuário*` ou `*⚠️ Conexão interrompida*`), salvando o progresso parcial.
*   **Operações de Escrita Seguras (`safeWrite` / `safeEnd`):** Métodos que encapsulam as saídas do stream HTTP para evitar falhas ou estouro de pilha ao tentar gravar em sockets que já foram desconectados pelo cliente.
